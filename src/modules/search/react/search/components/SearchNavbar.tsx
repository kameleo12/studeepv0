...
import { images } from "@root/assets/images";
import { useResetSearch } from "@root/modules/search/react/search/hooks/use-reset-search.hook";

export default function SearchNavbar() {
  const pathname = usePathname();
  const { handleResetSearch } = useResetSearch();
          ...
          <div className="flex px-2 lg:px-0">
            ...
            <div className="hidden lg:ml-6 lg:flex lg:space-x-8">
              ...
              <Link
                href={`/star-search`}
                className={
                  isActive(`/star-search`) ? activeClass : inactiveClass
                }
              >
                Star Videos
              </Link>
              <Link
                href={`/libraries`}
                className={isActive(`/libraries`) ? activeClass : inactiveClass}
              >
                Libraries
              </Link>
            </div>
