...
export const selectAuthorMostViral: (
  state: AppState
) => SearchResultsVM.TiktokMedia[] = createSelector(
  (state: AppState) => state.currentMedia.authorMostViral || [],
  (medias) => {
    if (!medias || medias.length === 0) {
      console.warn("selectAuthorMostViral - No data available");
      return [];
    }
