...
import { StarSearchResultsVM } from "@root/modules/search/core/models/star-search-results.view-model";
...
export const selectStarSearchResults: (
  state: AppState
) => StarSearchResultsVM.Display = createSelector(
  (state: AppState) => state.starSearch,
  (starSearchState) => ({
    hasStarted: true,
    isLoading: starSearchState.loading,
    medias: starSearchState.results.map((media) => ({
      id: media.id,
